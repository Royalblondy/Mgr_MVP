...
print("2. Przygotowanie danych dla LSTM...")
...
# --- KLUCZOWY MOMENT: ROZDZIELENIE CZASOWE DLA HYBRYDY ---
mask_lstm_train = years_seq <= 2015
mask_xgb_train = (years_seq > 2015) & (years_seq <= 2019)  # <-- XGB uczy się na nowych błędach sieci
mask_test = years_seq >= 2020

X_train_lstm, y_train_lstm = X_seq[mask_lstm_train], y_seq[mask_lstm_train]

print("3. Trenowanie ekstraktora cech (LSTM) na latach <= 2015...")
...
print("4. Generowanie szczerych przewidywań LSTM...")
...
print("5. Fuzja danych dla XGBoost...")
df_hybrid = pd.merge(df_clean, lstm_preds_df, on=['Area', 'Year'])
df_hybrid['Area'] = df_hybrid['Area'].astype('category')

# Tworzymy zbiory dla XGBoosta opierając się na naszej logice kaskadowej
...
print(f"6. Trenowanie XGBoost (tylko na latach 2016-2019, rekordów: {len(X_train_xgb)})...")
...
print("\n7. Ostateczne Testy na nieznanych latach 2020+...")
...
print("\n=== WYNIKI MODELU HYBRYDOWEGO V2 (BEZ WYCIEKU) ===")
print(f"MAE:  {mae:.4f}")
